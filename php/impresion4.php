...
if(!empty($_GET['Nucliente'])){
    ...
    FROM `factura_abono` 
    WHERE `id_cliente1` = '$Nucliente' AND `metodo_abono` != ''") or die ("Error al consultar: no se obtuvo la identificacion del usuario");
    ...
    ///////////////////////////////////////////////////////////
    //traemos el ultimo abono

    $consulta = mysqli_query($conexion, "SELECT `abono`, `fecha_abono`, `metodo_abono`
    FROM `factura_abono` 
    WHERE `id_cliente1` = '$Nucliente' AND `metodo_abono` != ''
    ORDER BY `id_factura_abono` 
    DESC LIMIT 1;") or die ("Error al consultar: no se obtuvo la identificacion del usuario");
    ...
    while (($fila = mysqli_fetch_array($consulta))!=NULL) {
        ...
        $metodo_abono   = $fila['metodo_abono'];
    }
    mysqli_free_result($consulta); //Liberar espacio de consulta cuando ya no es necesario
    ...
    <div style="text-align: left; text-transform: uppercase;font-size:0.8em">
        <table style="border-collapse: collapse">
            ...
            <br>
            <tr>
                <td width="85%" colspan="3">>>&nbsp;Abona</td> 
                <td width="15%"><?php echo number_format($abono, 0, ',', '.'); ?></td>  
            </tr>
            <tr>
                <td width="85%" colspan="3"><?php echo $metodo_abono ?></td> 
                <td width="15%"></td>  
            </tr>
            <tr>
                <td colspan="4">---------------------------------------------------------------</td> 
        ...
        <table style="border-collapse: collapse">
            <tr>
                <td width="40%">Registro:</td> 
                <td width="60%"><?php echo $fecha_abono ?></td>  
            </tr>
            <tr>
